...
##########################################################################
'''
def extractLesions2(timeIndex, subjectFolder):
    lesionSurfaceDataFilePath = subjectFolder + "\\surfaces\\lesions\\lesions" + str(timeIndex) + ".vtm"
...
crasFileName = rootFolder + "meta\\cras.txt"
dataCount = 81
...
for timeIndex in range(dataCount):    
    ...
    lesionActors = extractLesions2(timeIndex, rootFolder)
    # Read fiber tract data
    fiberTractFileName = rootFolder + "\\fibertracts\\" + str(timeIndex) + ".vtk"
    ...
    writer.SetFileName(rootFolder + "surfaces\\streamlinesMultiBlockDatasetDTI" + str(timeIndex) + ".xml")
    writer.SetInputData(mb)
    writer.Write()
    print("Processed:", timeIndex, "BLOCKDATASET WRITE SUCCESS")